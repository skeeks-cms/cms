...
class RbacController extends Controller
{
    ...
    /*protected function _initBaseData()
    {
        ...
        //Привилегия доступу к админке
        if (!$adminAccess = $auth->getPermission(CmsManager::PERMISSION_ADMIN_ACCESS))
        {
            $adminAccess = $auth->createPermission(CmsManager::PERMISSION_ADMIN_ACCESS);
            $adminAccess->description = 'Доступ к системе администрирования';
            $auth->add($adminAccess);
        }
        ...
        if (!$manager = $auth->getRole('editor'))
        {
            //Редактор, управляет только своими записями
            $manager = $auth->createRole('editor');
            $manager->description = 'Редактор';
            $auth->add($manager);
            $auth->addChild($manager, $adminAccess);
        }
