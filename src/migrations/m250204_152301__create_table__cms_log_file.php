...
class m250204_152301__create_table__cms_log_file extends Migration
{
    public function safeUp()
    {
        $tableName = 'cms_log_file';
        ...
        $this->createTable($tableName, [
            ...
            'cms_log_id' => $this->integer()->notNull(),
        ...
        $this->createIndex($tableName.'__cms_log_id', $tableName, ['cms_log_id']);
        $this->createIndex($tableName.'__storage_file_id', $tableName, ['storage_file_id']);

        $this->createIndex($tableName.'__uniq', $tableName, ['storage_file_id', 'cms_log_id'], true);

        $this->addCommentOnTable($tableName, 'Файлы к заметкам');
        ...
        $this->addForeignKey(
            "{$tableName}__cms_log_id", $tableName,
            'cms_log_id', '{{%cms_log}}', 'id', 'CASCADE', 'CASCADE'
        );
