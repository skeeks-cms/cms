...
class m160222_203220__alter_table__cms_content extends Migration
{
    public function safeUp()
    {
        ...
        $this->addColumn('{{%cms_content}}', 'parent_content_on_delete',
            $this->string(10)->notNull()->defaultValue("CASCADE"));
        $this->createIndex('parent_content_on_delete', '{{%cms_content}}', 'parent_content_on_delete');

        $this->addColumn('{{%cms_content}}', 'parent_content_is_required',
            $this->string(1)->notNull()->defaultValue("Y"));
